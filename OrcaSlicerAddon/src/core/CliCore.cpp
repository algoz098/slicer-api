    ...
    bool performSlicing(const std::string& output_file) {
        ...
        try {
            ...
            if (export_3mf) {
                ...
                sp.strategy = Slic3r::SaveStrategy::Silence | Slic3r::SaveStrategy::SplitModel | Slic3r::SaveStrategy::WithGcode | Slic3r::SaveStrategy::SkipModel | Slic3r::SaveStrategy::Zip64 | Slic3r::SaveStrategy::WithSliceInfo;
                ...
                try {
                    auto make_invalid_tn = [](){ auto *t=new Slic3r::ThumbnailData(); t->reset(); return t; };
                    ...
                    std::ifstream ifs(tmp_gcode, std::ios::in); if(ifs){
                        ...
                        if(!segs.empty()){
                            ...
                            // For single-plate export, only include the selected plate's thumbnail as index 0
                            sp.thumbnail_data.clear();
                            sp.thumbnail_data.push_back(tn);
                            // Ensure relationships reference the generated thumbnail (plate_1.png)
                            sp.export_plate_idx = 0;
                        }
                    }
                } catch (...) { /* non-fatal */ }
