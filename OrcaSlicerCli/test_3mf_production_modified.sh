...
# Teste de exportação 3MF (produção) com G-code embutido para o projeto MODIFICADO
# - Gera output .gcode.3mf para plate 1
# - Extrai Metadata/plate_1.gcode do 3MF e compara com arquivo de referência
...
INPUT_3MF="$SCRIPT_DIR/../example_files/3DBenchy_modified.3mf"
...
if [ ! -x "$CLI_BIN" ]; then
  ...
  (
    ...
    /Applications/CMake.app/Contents/bin/cmake --build . -j4
  )
fi
...
# Normaliza CONFIG_BLOCK removendo linhas não determinísticas/decoração que não afetam o fatiamento
normalize_config_block() {
  sed -E \
    -e '/^;[[:space:]]*different_settings_to_system[[:space:]]*=.*/d' \
    -e '/^;[[:space:]]*internal_solid_infill_speed[[:space:]]*=.*/d'
}
...
compare_gcodes() {
  ...
  # Não falhar apenas por diferença no total de linhas; validamos conteúdo normalizado abaixo
  ...
  ref_cfg=$(extract_config_block "$ref" | normalize_config_block)
  gen_cfg=$(extract_config_block "$gen" | normalize_config_block)
...
PLATE=1
OUT_3MF="$OUT_DIR/output_3DBenchy_modified_plate_${PLATE}.gcode.3mf"
REF_GCODE="$REF_DIR/3DBenchy_plate_${PLATE}_modified.gcode"
EXTRACTED_GCODE="$OUT_DIR/extracted_modified_plate_${PLATE}.gcode"

echo "\n==== Exportando 3MF (modified, plate $PLATE) ===="
...
if [ -f "$REF_GCODE" ]; then
  if compare_gcodes "$REF_GCODE" "$EXTRACTED_GCODE" "3MF modified plate $PLATE"; then
    echo "✅ [3MF modified plate $PLATE] G-code embutido idêntico ao de referência (com normalizações permitidas)."
  else
    echo "❌ [3MF modified plate $PLATE] Diferenças detectadas no G-code embutido."
    STATUS=1
  fi
else
